    ...
    if (!$search=='')
    {
        ...
        // và chuyển hướng về trang danh sách user
            ...
            <script language="javascript">
                window.location = '<?php echo create_link(base_url('admin'), array('m' => 'user', 'a' => 'searchdevice')); ?>';
            </script>
            <?php
        }
        else{
            ...
            ?>
            <div class="controls">
                <a class="button" href="<?php echo create_link(base_url('admin'), array('m' => 'user', 'a' => 'listdevice')); ?>">back</a>
            </div>
            <?php
...
<?php
        ...
        $sql = db_create_sql("SELECT * FROM Devices");
...
<center><h1>List of Device</h1></center>
        ...
        <br> -->
<div class="controls">
<a class="btn btn-primary btn-sm" role="button" style=" " href="<?php echo create_link(base_url('admin'), array('m' => 'user', 'a' => 'adddevice')); ?>">Add</a>
...
<table  class="table table-hover datatables">
    ...
    <tbody>
        ...
        <tr class="danger">
            <td ><?php echo $item['key_device']; ?></td>
            <!-- <td ><?php echo $item['password']; ?></td> -->
            <td ><?php echo $item['description']; ?></td>
            <?php if (is_admin()){ ?>
            <td>
                <form method="POST" class="form-delete" action="<?php echo create_link(base_url('admin/index.php'), array('m' => 'user', 'a' => 'delete')); ?>">
                    <input type="hidden" name="key_device" value="<?php echo $item['key_device']; ?>"/>
                    <input type="hidden" name="request_name" value="delete_device"/>
...
<script language="javascript">
    $(document).ready(function(){
        ...
        $('.form-delete').submit(function(){
            if (!confirm('Bạn có chắc muốn xóa Device này không?')){
                return false;
            }
